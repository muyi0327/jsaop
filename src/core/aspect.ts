...
const isPromise = (fn) =>
  !!fn && typeof fn.then === "function" && fn[Symbol.toStringTag] === "Promise";
            ...
            value: function () {
              ...
              const executeChain = () => {
                ...
                if (pointcut instanceof PointcutClass) {
                  let before = pointcut.findAdvice(
                    "before"
                  ) as BeforeAdviceType;
                  let after = pointcut.findAdvice("after") as AfterAdviceType;
                  let around = pointcut.findAdvice(
                    "around"
                  ) as AroundAdviceType;
                  let afterThrowing = pointcut.findAdvice(
                    "afterThrowing"
